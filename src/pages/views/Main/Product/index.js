...
const Product = ({ categorys, fetchAllCategory }) => {
  ...
  const getAllProDucts = async () => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:8000/products?_limit=6`)
      setProducts(data)
    } catch (error) {
    }
  }
    ...
    <div>
      <div className="product-model">
        <div className="container">
          ...
          <h2>Our Products</h2>
          <Link to="/searchprice">Giá từ thấp tới cao</Link><br />
          <Link to="/searchname">tên từ a-z</Link>
